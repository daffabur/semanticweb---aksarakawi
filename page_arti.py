...
if "input_arti" not in st.session_state:
    st.session_state.input_arti = ""
# --- Hapus inisialisasi histori pencarian untuk halaman ini ---
# if "search_history_arti" not in st.session_state:
#     st.session_state.search_history_arti = []
...
def app():
    st.title("📚 Cari Berdasarkan Arti Bahasa Indonesia")
    st.markdown("---") # Pemisah antara judul halaman dan sidebar content

    # --- Sidebar untuk Input Teks dan Kontrol (Arti) ---
    with st.sidebar:
        st.header("📝 Masukkan Arti Bahasa Indonesia")
        st.session_state.input_arti = st.text_input(
            "Arti Bahasa Indonesia:",
            value=st.session_state.input_arti,
            placeholder="Contoh: nama kerajaan",
            key="arti_sidebar_input_field", # Key unik
            label_visibility="collapsed"
        )

        # Tombol cari di sidebar
        if st.button("🚀 Cari Arti di Database", key="arti_sidebar_cari_button", use_container_width=True, type="primary"):
            if st.session_state.input_arti.strip() == "":
                st.warning("⚠️ Masukkan arti terlebih dahulu.")
            else:
                with st.spinner(f"Mencari data untuk '{st.session_state.input_arti}'..."):
                    results = perform_sparql_query("arti", st.session_state.input_arti)
                    if results:
                        ...
                        if bindings:
                            ...
                            # st.session_state.search_history_arti.append({
                            #     "input": st.session_state.input_arti,
                            #     "results": bindings
                            # })
                            ...
                            st.error(f"❌ Tidak ditemukan data untuk arti: **{st.session_state.input_arti}** di database.")
                    else:
        ...
        if st.button("🗑️ Bersihkan Input Arti", key="arti_sidebar_clear_button", use_container_width=True):
            st.session_state.input_arti = ""
            st.rerun()

        # --- Bagian Histori Pencarian Arti DIHAPUS ---
        # if st.session_state.search_history_arti:
        #     st.markdown("---")
        #     st.subheader("📜 Histori Pencarian Arti")
        #     for i, history in enumerate(st.session_state.search_history_arti):
        ...
        st.caption("Panel Kontrol Arti Bahasa Indonesia")
    ...
    st.info("Anda bisa mengetik arti dalam Bahasa Indonesia di sidebar kiri, lalu tekan tombol 'Cari Arti di Database'.")
    st.markdown("---")